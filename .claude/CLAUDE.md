...
- **PostToolUse는 `Write|Edit|MultiEdit`만 본다(Claude Code 계약).** Bash로 들어온 변경은 훅에 안 잡히므로, 그 구멍은 다른 층에서 메운다 — Stop 훅이 Bash 명령까지 보고(`_common.bash_wrote_files`), 비밀키는 `check`의 저장소 전체 스캔이 유일한 그물이다. **훅에만 의존하는 안전장치를 새로 만들지 말 것.** (`pre_commit_gate.py`가 `PreToolUse`로 Bash를 보지만 **`git commit`만** 본다 — Bash 구멍이 메워진 것이 아니다.) `bash_wrote_files`는 **대표적인 쓰기 명령을 추정할 뿐 완전하지 않다**(`perl -pi`·`git apply`·프로젝트 전용 CLI·빌드 도구는 빠진다) — 문서에 "Bash 수정도 전부 즉시 검사한다"고 쓰지 말 것. <!-- hi-vibe: allow-overclaim (금지 문구 인용) -->
...
- **fresh-eyes 강제는 Stop(턴 끝)에서 `PreToolUse`(커밋 직전)로 옮겼다** (2026-09-13).
  턴 끝에 세우면 **리뷰가 자기 일을 해낼수록 다음 차단을 스스로 만든다** — 리뷰가
  파일을 고치면 내용 지문이 달라져 "같은 변경은 두 번 막지 않기"가 무력해진다(실측
  2시간 세션 5회 차단 · 서브에이전트 9개 중 7개 · 사용자 요청은 3건). 커밋은 훅이
  관측할 수 있는 **유일한 "작업 끝" 신호**다 — 사용자가 직접 찍으므로 근사가 아니고,
  커밋 시도는 한 번이라 고리가 원리적으로 안 생긴다.
  **범위는 staged가 아니라 "안 커밋한 변경 전부"다 — 되돌리지 말 것**(2026-09-13에
  좁혔다가 같은 날 고쳤다). `PreToolUse`는 명령이 **실행되기 전에** 걸리는데 실제
  커밋은 `git add -A && git commit` 한 줄인 경우가 압도적이라, 그 시점의 인덱스는
  비어 있다 — 게이트가 "커밋할 코드 없음"으로 조용히 통과시켰다(실측: 커밋 8개 중
  7개가 이 형태, 게이트 설치 후 3커밋 전부 리뷰 없이 통과). 대가는 알고 고른
  것이다: 커밋에 안 들어갈 지저분한 파일까지 범위에 든다. **자가진단도 stage하지
  않은 상태로 물어야 한다** — 예전 `doctor`는 `git add` 뒤에 물어서 이 구멍을
  그대로 통과시켰다.
  **되돌리지 말 것**: 예전 문턱("표시한 파일 2개 이상")과 `FRESH_EYES_MIN_FILES`는
  이 이동으로 사라졌다. 대가를 알고 고른 것이다 — **커밋을 안 하는 흐름에서는
  fresh-eyes가 기계로 강제되지 않는다.** 체크리스트(싼 층)는 그대로 턴 끝에 남는다.
  `pre_commit_gate.py`는 **커밋 경계마다 한 번만** 막는다: Agent 호출이 막힌 환경
  (2026-08-07에 겪었다)에서 커밋이 영구 불가능해지는 쪽이 못 막는 것보다 나쁘다.
  커밋 명령 탐지는 `bash_wrote_files`와 같은 **근사**다(별칭·래퍼는 빠진다) —
