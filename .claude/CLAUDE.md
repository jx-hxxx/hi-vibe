...
- **동작 하나를 설명하는 문장은 열 곳까지 산다.** README 한/영 · 랜딩 한/영 · 해당 스킬 · 템플릿 · **훅 안의 문자열 상수** · 명령 `.md` · **`scripts/doctor.py`(문구와 실행 목록 양쪽)**. 훅을 하나 늘렸더니 "훅 4종"이 정확히 열 곳에 있었다. 동작을 바꾸면 그 전부를 봐야 한다. 실제로 `handover`가 무엇을 남기나 하나를 고치는 데 세 릴리스가 걸렸다 — 훅 문자열과 랜딩 "세 겹"을 몰랐기 때문이다.
