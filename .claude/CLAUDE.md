...
- **Stop 훅이 fresh-eyes를 강제하는 문턱은 "표시한 파일 2개 이상"이고, 1로 내리지 말 것** (2026-08-17). 예전엔 `mark`가 잠금을 푸는 **유일한 열쇠**라 체크리스트만 돌리고 표시하면 훅이 만족했다 — 훅은 fresh-eyes 횟수를 세고 있었지만 **차단 판단에 안 쓰고 있었다**(`doctor`는 0회일 때만 경고해서 부분 누락은 조용히 지나갔다). 2를 고른 건 임의값이 아니라 fresh-eyes 1번 항목("고친 파일을 가리키는 다른 파일이 같이 바뀌었나")이 **파일이 둘 이상일 때만 성립**하기 때문이다. **한 파일짜리 리뷰는 기계로 안 막는 것이 의도된 한계다** — 1로 내리면 오타 하나 고칠 때도 서브에이전트를 부르게 되고, 그게 "잔소리 훅은 플러그인 삭제로 이어진다"는 이 훅의 원래 경계와 정면으로 부딪친다.
