...
- **답변 검사 훅 — 훅이 이제 내가 쓴 문장을 읽는다** (2026-09-05, `answer_gate.py`·`_answer_check.py`) — 코드에는 테스트와 fresh-eyes가 있는데 **대화에는 아무 장치가 없었다.** 같은 날 한 세션에서 격식체 지시를 여러 번 어기고, 비유로 사실을 흐리고, 확인하지 않은 동작을 단정하는 일이 반복됐다. 문서에 적어 두는 층으로는 안 된다는 것이 그날로 확인돼 기계 층으로 내렸다.
  ...
  - **근거는 막지 않고 드러낸다.** 저장소에 실재하는 파일을 지목해 설명하면서 그 턴에 아무것도 열지 않았으면 한 줄 표시만 남긴다. 막으면 통과하는 가장 싼 방법이 "아무 파일이나 한 번 여는 것"이 되고, 이 저장소는 이미 그 함정을 겪었다(리뷰 없이 `mark`만 해서 통과하던 것을 v0.48.0에서 막았다). 그리고 "이 문장이 동작을 단정하는가"는 기계가 판정할 수 없다 — 판정할 수 없는 것으로 막으면 오탐이 쌓이고, 오탐이 쌓이면 사람이 검사를 끈다.
  - **막는 것과 표시하는 것을 가르는 기준**: 막혔을 때 할 일이 분명한가. 말투는 다시 쓰면 되고 그 능력은 이미 있다. 근거는 다시 쓴다고 아는 것이 아니라 열어 봐야 아는 것이라, 강제하면 형식만 채우게 된다.
