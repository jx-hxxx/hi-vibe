...
## [0.54.1] - 2026-09-13
<!-- show:ko **커밋 직전 검토가 한 번도 안 걸리고 있었습니다.** 어제 넣은 게이트를 실사용 기록으로 확인해 보니, 커밋 8개 중 7개가 `git add -A && git commit` 한 줄이었어요. 훅은 그 줄이 **실행되기 전에** 끼어들어 "무엇이 커밋되나"를 보는데, 파일을 올리는 일이 바로 그 줄 안에 있으니 그 시점엔 올라간 파일이 하나도 없습니다. 게이트는 "커밋할 코드가 없네"로 읽고 조용히 통과시켰어요 — 훅이 죽은 게 아니라 헛다리를 짚은 겁니다. 그래서 **"올라간 파일"이 아니라 "아직 커밋 안 된 변경 전부"를 보도록** 고쳤습니다. **대가도 밝혀 둡니다: 문서 한 줄만 커밋해도 작업하다 만 코드가 남아 있으면 걸립니다.** 억울할 땐 "그냥 커밋해"라고 하면 지나가요(같은 커밋은 두 번 막지 않습니다). 자가진단(`doctor`)도 일부러 stage하지 않은 상태로 검사하도록 바꿨습니다 — 예전 검사는 이 구멍을 그대로 통과시켰거든요. -->
<!-- show:en **The pre-commit review was never once triggering.** Checking yesterday's gate against real session records: seven of eight commits were a single `git add -A && git commit` line. The hook runs **before** that line executes and asks "what is about to be committed" — but the staging happens inside the very same line, so at that moment nothing is staged. The gate read "no code to commit" and waved it through: not dead, just looking at the wrong thing. It now looks at **everything uncommitted**, not only what is staged. **The cost, stated plainly: committing a one-line doc change is blocked if half-finished code is still lying around.** Say "just commit it" and it passes (the same commit is never blocked twice). The self-check (`doctor`) now probes with nothing staged — the old probe sailed straight through this hole. -->

### Fixed
- **커밋 게이트가 가장 흔한 커밋 형태에서 통째로 샜다** (2026-09-13, `pre_commit_gate.py`·`review_scope.py`) — `PreToolUse`는 Bash 명령이 **실행되기 전에** 걸린다. `git add -A && git commit ...`처럼 스테이징이 커밋과 같은 줄에 있으면 그 시점의 인덱스는 비어 있어, `staged` 범위가 빈 목록을 돌려주고 게이트는 "커밋할 코드 없음"으로 통과시켰다. **실측**: 한 프로젝트 세션의 커밋 8개 중 7개가 이 형태였고, 게이트 설치 후의 커밋 3건이 전부 fresh-eyes 없이 나갔다(훅은 정상 실행됐다 — `heartbeat.json`의 `PreToolUse`가 마지막 커밋과 초 단위로 일치했고, `commit-gate.json`이 안 생긴 것이 범위 판정 전에 return했다는 증거였다).
  - `review_scope.py`의 `staged` 모드를 **`precommit`으로 바꿨다** — stage 여부를 안 보고 안 커밋한 변경 전부(추적 변경 + untracked)를 돌려준다. 이름을 그대로 두면 하는 일과 이름이 갈린다.
  - **대가는 알고 고른 것이다**: 커밋에 안 들어갈 지저분한 파일까지 범위에 든다(문서만 stage해도 미완성 코드가 남아 있으면 막힌다). 정확한 범위와 실제로 잡히는 것 중 후자를 택했다 — 억울한 차단은 "그냥 커밋해"로 한 번에 지나가지만, 안 잡히는 것은 지나간 줄도 모른다.
  - `doctor`의 커밋 게이트 검사를 **stage하지 않은 상태**로 바꿨다. 예전 검사는 `git add` 후에 물어봤기 때문에 이 구멍을 그대로 통과시켰다 — 자가진단이 실제 사용 형태를 흉내 내지 않으면 죽은 것을 못 잡는다.
  - 회귀 테스트 2건 추가(구멍 자체 · 알고 고른 대가), 문구가 어긋난 테스트 2건 갱신(347→349개).
...
- **답변 검사 훅 — 훅이 이제 내가 쓴 문장을 읽는다** (2026-09-05, `answer_gate.py`·`_answer_check.py`) — 코드에는 테스트와 fresh-eyes가 있는데 **대화에는 아무 장치가 없었다.** 같은 날 한 세션에서 격식체 지시를 여러 번 어기고, 비유로 사실을 흐리고, 확인하지 않은 동작을 단정하는 일이 반복됐다. 문서에 적어 두는 층으로는 안 된다는 것이 그날로 확인돼 기계 층으로 내렸다.
  ...
  - **비유는 종결어미로 안 잡힌다.** "체결이 흐르는 차트가 뜨고"는 `않습니다`로 멀쩡히 끝나면서 틀린다. 여기만 금지 목록인데, 사실 서술 동사를 전부 나열할 수 없어 허용 목록이 성립하지 않기 때문이다. **새는 것을 전제로 목록을 코드 밖 파일(`informal_words.txt`)에 뒀다** — 지적받으면 한 줄 더하면 되고 파이썬을 고칠 필요가 없다.
  - **근거는 파일 단위로 본다.** 답변이 저장소에 실재하는 파일을 지목해 설명하면서 **그 파일을** 이번 턴에 열지 않았으면 막는다.
...
- **등록한 훅을 doctor가 전부 돌리는지 검사한다** (2026-09-05, `test_answer_gate.py`) — `hooks.json`의 목록과 `doctor.HOOK_SCRIPTS`가 갈리면 죽은 훅을 doctor가 통과시킨다. CLAUDE.md가 경고한 "한 동작이 열 곳에 산다"의 기계 방지선이다.
