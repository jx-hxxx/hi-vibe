...
```
/hi-vibe:doctor   ← 설치가 잘 됐나 점검 (프로젝트 상관없이 1회)
/hi-vibe:init     ← 이 프로젝트에 문서 시스템 설치 (폴더마다 1회)
   (뭐부터 할지 모르겠으면 /hi-vibe:welcome 먼저 보세요)
```

**② 그 다음엔 그냥 평소처럼 코딩하세요 — 나머진 자동이에요**

명령어를 순서대로 칠 필요 없어요. 평소처럼 말하면 알아서 돌아가요:

- "이 함수 만들어줘" → **pre-write** 자동 발동 (이미 있는지 먼저 검색)
- "다 했어 / 리뷰해줘" → **post-write** 자동 발동 (품질 체크리스트)
- "설계도 봐줘" → **post-write --deep** (이 코드를 쓴 기억이 없는 깨끗한 AI를 불러 "과잉 설계·불필요한 기능" 같은 판단 착오를 잡아줌)
- 컴팩트할 때마다 → **handover** 자동 기록
- 실질 변경하면 → **CHANGELOG** 자동 기록
- 에러 삼킴·비밀키 하드코딩 → 쓰는 순간 자동 감지
...
> "컨텍스트 짧게 유지 + 핸드오버 자주"가 손 안 대고 됩니다.
> 단, 이 환경변수는 **모델·환경에 따라 효과가 제한**될 수 있어요(공식 문서
> 기준). 켠 뒤 실제로 30%에서 도는지 한 번 확인하세요. 훅이 스스로
> `/compact`를 누를 수는 없어서, 이 환경변수가 유일한 자동화 방법이에요.

## audit · guards 더 알아보기 (위 ③의 선택 명령)
