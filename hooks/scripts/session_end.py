...
PreCompact와 다른 점 둘:
  ...
  2. **같은 세션에서 이미 남겼으면 건너뛴다.** `/compact` 하고 바로 `/clear`를
     치는 흐름이 흔한데, 그때 거의 같은 내용이 두 번 들어간다.
...
# 왜 끝났는지에 따라 사람이 읽을 제목이 달라진다. 여기 없는 이유(logout 등)는
# 매처에서 안 받는다 — 늘리는 건 실제로 아쉬운 경우가 나온 뒤에.
REASON_LABEL = {
    "clear": "/clear 직전",
...
def main(payload):
    ...
    if _common.handover_already_written(cwd, sid, n_edited):
        return  # 방금 compact이 같은 내용을 남겼다
    ...
    _common.note_handover_written(cwd, sid, n_edited)
