...
def tone_reason(bad, figs):
    ...
    if bad:
        ...
        parts.append("[격식체로 끝나지 않음]\n" + shown + more)
    if figs:
        shown = "\n".join(f"  - {s}   ← `{w}`" for s, w in figs[:5])
        more = f"\n  (그 밖 {len(figs) - 5}개)" if len(figs) > 5 else ""
        parts.append("[비유 — 사실 서술로]\n" + shown + more)
    # 답변 전체를 다시 내게 하면 사용자 화면에 같은 뭉텅이가 두 번 찍힌다
    # (앞 답은 이미 나갔고 block은 그걸 지우지 못한다). 걸린 문장만 고친
    # 형태로 내는 게 사용자에게 보이는 잡음이 가장 적다 (2026-09-11 사용자 지적).
    parts.append(
        "**답변 전체를 다시 출력하지 마세요** — 앞 답은 이미 화면에 있습니다. "
        "걸린 문장만 고친 형태로 짧게 내고, 사과·해명은 붙이지 마세요. "
...
def evidence_reason(hits):
    """차단 사유 = 열어 보라는 지시. 어느 파일인지만 주고 짧게 끝낸다."""
    shown = ", ".join(f"`{h}`" for h in hits[:6])
    more = f" (그 밖 {len(hits) - 6}개)" if len(hits) > 6 else ""
    return (
        f"hi-vibe 근거 검사: {shown}{more}를 설명하면서 이번 턴에 그 파일을 "
        "열지 않았습니다.\n"
        "지금 그 파일을 열고 본 것과 답이 같은지 확인하세요. **답변 전체를 다시 "
        "출력하지 마세요** — 같으면 근거가 된 줄을 한 줄로 밝히고, 다르면 달라진 "
        "부분만 고쳐 쓰고, 스치듯 언급한 것뿐이면 그렇다고 한 줄로 밝히세요."
    )
