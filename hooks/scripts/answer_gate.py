...
def tone_reason(bad, figs):
    ...
    이미 무엇이 문제인지 말한다."""
    parts = ["hi-vibe 말투 검사 — 아래를 고쳐 답변을 다시 쓰세요."]
    if bad:
    ...
    parts.append(
        "사과·해명 없이 고친 답만 내고, 그 자리에서 지어낸 축약어가 있으면 "
        "함께 풀어 쓰세요. 코드 블록·인용·목록 라벨은 검사 대상이 아닙니다."
    )
    return "\n".join(parts)
...
def evidence_reason(hits):
    ...
    return (
        ...
        "열지 않았습니다.\n"
        "지금 그 파일을 열고 본 것과 답이 같은지 확인한 뒤 답하세요 — 다르면 "
        "고치고, 같으면 근거가 된 줄을 밝히세요. 스치듯 언급한 것뿐이면 그 "
        "이름을 빼고 다시 쓰세요."
    )
