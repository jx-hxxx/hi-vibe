...
122개 · 프로젝트별 활성화 · Python 표준 라이브러리 기반 핵심 기능**으로
...
100% 보장되지는 않습니다. 확실히 실행하고 싶다면 `/hi-vibe:find`처럼 해당
명령을 직접 호출하세요.

`review`는 예외입니다. AI가 알아듣고 돌기도 하지만, 놓치면 **턴이 끝날 때
Stop 훅이 붙잡아 직접 돌립니다**. AI 기억에 기대지 않는 쪽이 뒤를 받칩니다.
