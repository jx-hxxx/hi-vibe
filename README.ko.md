...
- 😵 세션이 바뀌면 기억을 잃고, 이미 만든 **함수·헬퍼·타입(자료구조)을 비슷하게 또 만들어요**
- 🩹 에러가 나면 원인 대신 try/except로 삼키거나 **fallback·기본값으로 땜빵해요** (그래서 버그가 조용히 숨어요)
- 🤷 **해석이 여러 갈래인 요청·빠진 요구사항을** 묻지도 않고 마음대로 진행해요
- 📊 확인도 안 한 **API 제한·가격·버전을** **공식 스펙처럼** 단정해요
- 🗿 **엣지케이스는 건너뛰고** 한 파일에 다 몰아넣어 **'갓파일'이 되고**, 문서는 코드와 **따로 놀아요**
...
> **3번 `/reload-plugins`를 꼭 실행하세요.** 설치만 하면 명령어·hook이
> 아직 안 켜져요. 이 줄을 쳐야 이번 세션에서 바로 활성화됩니다
> (Claude Code 전체 재시작은 필요 없어요).

> **필수 요구사항**: Python 3.8+ (`python3` 명령이 있어야 hook이 동작해요).
...
설치·적용이 끝나면 위 3개는 순서대로, 아래 2개는 필요할 때 (선택):

```
/hi-vibe:welcome    ← 뭐부터 할지 모르겠으면 여기
/hi-vibe:doctor     ← 설치 직후 1회: hook이 진짜 작동하는지 실행으로 확인
/hi-vibe:init       ← 새 프로젝트에서 1회: 문서 시스템 설치 + hook 활성화
/hi-vibe:check      ← (선택) 구조 진단: 중복·갓파일 찾기 — 언제든 반복
/hi-vibe:gate --ci  ← (선택) 품질 관문 설치: 프로젝트마다 1회 → 이후 push마다 자동
```
...
> hook은 어떤 경우에도 Claude Code를 방해하지 않도록 "조용히 실패"하게
> 설계돼 있어요. 그 대가로 python3가 없으면 **티 안 나게 꺼진 상태**가
> 될 수 있는데, `doctor`가 hook 4종과 스캐너를 실제로 돌려서 확인해 줍니다.

`init`을 하면 이 4개 문서가 생기고, hook이 이 프로젝트에서 활성화돼요:
...
/hi-vibe:gate --ci      ← 품질 관문 설치 (프로젝트마다 1회 → 이후 push마다 자동)
```

이 둘은 **쓰는 리듬이 반대**예요 — `check`는 **내가 돌리는 진단**(한 프로젝트
안에서 원할 때마다 반복), `gate`는 **프로젝트마다 한 번 깔아두면 알아서 지키는
관문**(그 프로젝트에 설치는 1회, 이후 자동).
...
- **`gate`** — 코드 자동 검사기(린트)를 **물어보고** 설치해요(기존 설정은 안
  ...
  실패**(통과 못 하면 못 올려요). **쓰려는 프로젝트마다 한 번이면 끝** —
  `--ci`를 깔면 그 프로젝트는 그 뒤론 명령어를 다시 칠 필요 없이 push마다
  GitHub이 자동으로 검문해요. (규칙을 바꾸고 싶을 때만 다시 돌리면 됩니다.)

## init한 뒤엔 전부 자동이에요

위 "첫 사용"에서 **`init`을 한 프로젝트**에서는, 아래가 전부 알아서
돌아가요 (hook과 문서 자동화는 `init`으로 켜지거든요). 명령어를 순서대로
...
| "설계도 봐줘" | **review --deep** — 짠 기억 없는 딴 클로드가 과잉설계 잡기 | 🤖 AI |

> **⚙️ 기계** = hook(파이썬)이 **보장**해요 — AI 기분과 무관하게 실제로 돌아요.
...
> **review 3가지.** 하나의 `review`에 옵션을 붙여 쓰는 구조예요.
> `review`는 **방금 만든 기능 하나**를, `review --all`은 **세션에서 바뀐 기능
> 전체**를 한 번에 검증해요(이미 리뷰했고 안 바뀐 파일은 자동으로 건너뜀 —
> 여러 개 몰아 만든 뒤 "전체 리뷰해줘"에 좋아요). `review --deep`은 **코드를 짠
> 기억이 없는 딴 클로드(별도 에이전트)를 새로 불러와요** — 짠 적이 없어서 편견
> 없이, 깨끗한 눈(fresh-eyes)으로 과잉설계·숨은 결합을 봐요(짠 본인은 못 보는
> 것). `--all`은 **얼마나 넓게**(하나→전체), `--deep`은 **누가 보나**(짠 클로드
> →딴 클로드)라 다른 축이에요 — `review --all --deep`도 됩니다.
...
| `/hi-vibe:doctor` | 설치 직후, 또는 hook이 도는지 의심될 때 | 🖐 직접 |
| `/hi-vibe:init` | 프로젝트마다 1회 (문서 시스템 설치) | 🖐 직접 |
| `/hi-vibe:find` | **"이 기능 만들어줘"** 할 때 — 만들기 전 있는지 검색 | ⚡ 자동 |
| `/hi-vibe:review` | **"다 했어 / 리뷰해줘"** 할 때 — 점검 + 문서 동기화 (`--all` 세션 전체 · `--deep` 딴 클로드가 재검토) | ⚡ 자동 |
...
| `/hi-vibe:gate` | 린트(코드 자동 검사기) 설치 — 안 좋은 코드를 기계가 막음 (**프로젝트마다 1회 → 자동**) | 🖐 직접 |

> **⚡ 자동** = 평소 자연어("만들어줘"·"다 했어")나 hook(컴팩트 등)으로 알아서 발동해요. 명령어는 확실하게 하고 싶을 때 누르는 버튼일 뿐이에요.
>
> **🖐 직접** = 필요할 때 직접 명령어를 쳐서 실행해요 (설치·점검·진단처럼 내가 원할 때).

### 뒤에서 뭐가 도나 — 스킬(엔진) ↔ 명령어(버튼)

쉬운 명령어는 **버튼**이고, 실제 일은 뒤의 **스킬(엔진)** 이 해요:
...
> **스킬은 Claude Code가 `/이름`으로도 자동 노출해요.** 내가 만든 건 `check`·`find`
> 같은 쉬운 명령어 10개인데, Claude Code가 스킬 6개를 각각 `/repo-xray`처럼
> 슬래시로 자동으로 열어둬요. 그래서 같은 엔진을 **① `check` 명령어 ② `/repo-xray`
> 슬래시 ③ "중복 찾아줘"라고 말하면 AI가 `Skill()`로** — 셋 다로 부를 수 있어요.
> `hi-vibe:` 접두사는 "이 플러그인 소속"이라는 이름표예요 (명령어·스킬 공통).
> `doctor`만 스킬 없이 hook·스캐너를 직접 돌리는 진단 명령이에요.
...
**Q. hook 설정을 바꿨는데 반영이 안 돼요.**
hook은 세션 시작 시 로드돼요. Claude Code를 재시작하세요. `/hooks`로 로드 상태를 볼 수 있어요.

**Q. hook이 진짜 돌고 있는지 어떻게 알아요?**
`/hi-vibe:doctor` — hook 4종과 스캐너를 실제로 실행해서 ✅/❌로 보여줘요.
hook은 실패해도 조용히 넘어가도록 설계돼 있어서, 이 명령이 유일한 확인 수단이에요.

**Q. 다른 프로젝트에서도 hook이 동작하나요?**
...
**안 올라가는 것**: `handover.md`·`handover-archive.md`(개인 세션 로그), `.hi-vibe/`(hook 상태), `.repo-xray/`(스캔 캐시). 이것들은 `init`이 `.gitignore`에 자동 추가해요. (handover를 팀과 공유하고 싶으면 `.gitignore`에서 그 줄만 빼면 돼요.)
