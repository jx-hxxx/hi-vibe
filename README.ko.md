...
140개 · 프로젝트별 활성화 · Python 표준 라이브러리 기반 핵심 기능**으로
...
| 리뷰 안 받은 코드 변경이 있는 채로 턴이 끝날 때 | 그 자리에서 리뷰를 돌림 (직접 칠 필요 없음) | ⚙️ 기계 |
...
**턴이 끝날 때 Stop 훅이 붙잡아 직접 돌립니다** — AI 기억에 기대지 않는 쪽이
뒤를 받칩니다. 사람이 챙겨야 하는 안전장치는 안전장치가 아니니까요.
...
| `/code-review` | `review` | **리뷰 내용은 겹칩니다.** 다른 건 시점입니다 — 아래 참고 |
...
자동 실행이 있었다가 빠졌습니다. hi-vibe는 Stop 훅으로 **리뷰 안 받은 변경이
있으면 그 자리에서 리뷰를 돌립니다.** 같은 변경으로 두 번 막지는 않고요.
치는 걸 잊어도 걸린다는 게 이 플러그인의 핵심입니다.

**그래서 정확한 위치는 이렇습니다.** "Claude Code에 없는 기능을 추가한다"가
아니라, **이미 있는 좋은 기능을 놓치기 쉬운 순간에 자동으로 연결하고,
그 결과를 프로젝트 기록으로 남깁니다.** 리뷰 품질 자체는 앞으로도 기본
`/code-review`가 더 좋아질 겁니다 — 그건 그쪽에 맡기는 게 맞습니다.
...
훅은 Write/Edit로 **새로 쓰는** 코드만 봅니다. Bash(heredoc·`sed -i`·생성
스크립트)로 들어온 키는 훅을 통과하므로, **이 스캔이 유일한 그물**입니다.
