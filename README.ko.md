...
> **⚠️ 오해 방지 — 이건 "버그 자동 탐지기"가 아니에요.**
> 코드를 훑어서 버그를 찾아주는 도구가 아니라, **AI가 대충 못 하게 만드는 안전벨트**예요.
> 버그를 잡는 건 결국 Claude의 추론이고, hi-vibe는 그 추론을 **빡세게 하도록 강제하는
> 규율·문서·기계 장치**예요. 세 결로 나뉘고, **켜지는 방식이 달라요:**
>
> - 🔒 **깔면 거저 (훅)** — `init`만 하면 코드 쓸 때마다 **에러 삼킴·비밀키를 기계가
>   자동으로** 잡고, 컴팩트마다 handover를 남겨요. *진짜 항상 매여있는 안전벨트.*
> - 🧭 **부르면 (find · review · check)** — "만들어줘 / 다 했어 / 중복 찾아줘"에 규율
>   체크리스트·구조 스캔이 돌아요. 강력하지만 **AI가 놓치면 안 걸려요.** *옆에서
>   잔소리하는 코치 — 무시하면 안 잡혀요.*
> - 🛡️ **원하면 (gate)** — 복잡도·순환의존을 린트로 막아요. 잡는 건 제일 많지만
>   **침습적이라** 준비됐을 때 1회 설치(필수 아님).
>
> 즉 **"자동으로 다 잡아주는 도구"가 아니라, "안 하던 규율을 강제로 하게 만드는 장치"**
> 예요. 기대를 여기 맞추면 실망이 없어요.
...
> **스킬은 Claude Code가 `/스킬이름` 슬래시로도 자동 노출해요.** hi-vibe가 제공하는
> 건 `check`·`find` 같은 쉬운 명령어 10개인데, 여기에 Claude Code가 스킬 6개를 각각
> `/repo-xray`처럼 슬래시로 자동으로 열어둬요. 그래서 같은 엔진을 **① `check` 명령어
> ② `/repo-xray` 슬래시 ③ "중복 찾아줘"라고 말하면 AI가 `Skill()`로** — 셋 다로 부를
> 수 있어요. `hi-vibe:` 접두사는 "이 플러그인 소속"이라는 이름표예요 (명령어·스킬 공통).
