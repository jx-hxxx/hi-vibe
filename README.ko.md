...
84개 · 프로젝트별 활성화 · Python 표준 라이브러리 기반 핵심 기능**으로
...
> **hi-vibe를 쓸 프로젝트마다 각각 `/hi-vibe:init`을 해야 켜집니다.**
> 플러그인은 한 번만 설치하지만, 자동 기능(에러·비밀키 감지, 자동 handover 등)은
> `/hi-vibe:init`으로 `.hi-vibe/` 마커가 만들어진 폴더에서만 동작합니다.
> **init 안 한 다른 프로젝트에는 전혀 개입하지 않습니다** — 원하지 않는 곳에서
> 훅이 도는 걸 막는 안전장치예요. (그래서 새 프로젝트에서 쓰려면 그 폴더에서 `init` 한 번.)
...
| `handover.md` | 다음 세션을 위한 진행상황(자동 기록) + 결정·맥락(직접/AI가 다듬어 기록) |
...
- `review --all` — 아직 커밋하지 않은(uncommitted) Python/JS·TS 코드 파일 검토 (커밋하면 범위에서 빠져요. 설정·문서 파일과 삭제된 파일은 제외)
- `review --deep` — **새 서브에이전트(fresh-eyes)를 호출**해 코드를 작성한 적 없는 깨끗한 눈으로, 편견 없이 설계를 재검토
- `review --all --deep` — 세션 전체 변경을 그 서브에이전트의 시선으로 한 번에 검토 (기존 프로젝트에 막 설치했을 때 특히 추천)
