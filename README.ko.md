...
104개 · 프로젝트별 활성화 · Python 표준 라이브러리 기반 핵심 기능**으로
...
- **범위** — 아직 커밋하지 않은 Python/JS·TS 코드 파일 (설정·문서 파일과
  ...
  뒤로 안 바뀐 파일은 건너뜁니다.
- **깊이** — 코드를 작성한 적 없는 **새 서브에이전트(fresh-eyes)**가 편견 없는
  눈으로 설계를 재검토합니다. 옵션이 아니라 **기본**입니다. 설계랄 게 없는
  작은 변경일 때만 생략하고, 생략하면 생략했다고 밝힙니다.
- **병렬** — 변경이 크면 파일 수와 변경 줄 수를 재서 여러 리뷰어에게 나눠
  맡깁니다. 고르라고 묻지 않고, 그렇게 한다는 것과 토큰을 더 쓴다는 것을
  알린 뒤 진행합니다.

깊게 안 봐도 될 땐 "가볍게 봐줘"라고 말하면 됩니다. 범위도 마찬가지로
"로그인 쪽만"처럼 말로 좁힙니다.

fresh-eyes는 체크리스트만으로 잡기 어려운 과잉설계, 불필요한 기능, 숨은 결합,
지나친 추상화를 살펴봅니다.

**보통은 직접 칠 일이 없습니다.** 리뷰 안 받은 코드 변경이 있으면 Stop 훅이
알아서 돌려줍니다 (아래 참고).
...
| `/hi-vibe:check` | 구조 점검 + 하다 만 것 — 서브에이전트가 후보를 검증해서 줄여줌 | 🖐 직접 |
