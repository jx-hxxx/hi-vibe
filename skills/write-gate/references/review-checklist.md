...
이 파일은 `write-gate`의 review 모드에서만 읽는다. find 모드에서는 읽지 않는다.
아래 계약은 기존 리뷰 동작의 상세 기준이며, 요약본과 충돌하면 이 문서가 우선한다.
...
### 2. 기본은 순차 — 병렬은 정말 클 때만, 최대 2개

**기본값이 순차다.** 예전에는 "파일 여러 개 + 수백 줄"이면 버킷 3~6개를 병렬로
띄웠는데, 그 문턱이 너무 낮아서 보통 크기의 작업마다 리뷰어 3개 + fresh-eyes가
붙었다. 실측(MoToo 2026-09-12): 2시간 세션에서 이 세트가 두 번 돌아 서브에이전트
7개를 썼고, 사용자 요청은 3건이었다. 리뷰가 작업보다 비쌌다.

깊이가 걱정되어 병렬을 기본으로 뒀던 것인데, **얕아지는 것보다 리뷰 때문에
작업을 못 하게 되는 쪽이 실제로 일어난 실패다.**

- **순차로 본다** — 파일 몇 개든, 수백 줄이든. 대부분 여기서 끝난다.
- **병렬은 정말 클 때만**: 어림잡아 **파일 15개 이상이면서 1,500줄 이상**.
  그때도 **버킷은 최대 2개**다. 3개 이상으로 쪼개지 않는다.
- 병렬로 갈 때는 고르라고 묻지 말고 한 줄로 알리고 진행한다:

> 변경이 매우 큽니다 (23파일 2,100줄). 얕아지지 않게 둘로 나눠 봅니다.

`python3 "${CLAUDE_PLUGIN_ROOT}/skills/write-gate/scripts/review_scope.py" chunk 2 --root <repo>`
로 줄 수 균형이 맞은 `buckets` 2개를 받아 버킷마다 리뷰어를 하나씩 소환하고,
결과를 모아 통합 보고한다.

사용자가 "가볍게 봐줘"(영어로는 "keep it light" / "light review")라고 했으면
분할 없이 체크리스트만 돌린다 — 끄는 건 플래그가 아니라
...
### 4. fresh-eyes — 부르는 자리가 둘뿐이다

**① 커밋 게이트가 막았을 때** (자동 경로) · **② 사용자가 직접 요청했을 때**
(`/hi-vibe:review`·"설계 검토해줘"·"남의 눈으로 봐줘"). 명시 요청은 예산보다
우선하므로 ②에서는 망설이지 말고 부른다.

**훅이 턴 끝에 세운 리뷰(Stop 차단)에서는 부르지 않는다.** 체크리스트를
마쳤다는 것만으로 소환하지 않는다 — 그 자동 강제는 `pre_commit_gate.py`가
사용자가 **"커밋해줘"라고 말한 지점**에서 한 번 세운다.

왜 옮겼나: 턴이 끝날 때마다 세우면 **리뷰가 자기 일을 해낼수록 다음 차단을
스스로 만든다** — 리뷰가 파일을 고치면 변경 내용이 달라지고, 그러면 "같은
변경은 두 번 막지 않는다"는 방어가 무력해진다. 실측 2시간 세션에서 5회
차단, 서브에이전트 9개 중 7개가 이 고리에서 나왔다(2026-09-12). 체크리스트는
싸서 매 턴 돌아도 되지만, 이건 작업당 한 번이면 된다.

**부를 때** — 커밋이 막힌 경우 사유에 파일 목록이 온다(안 커밋한 변경 전부다 —
stage 여부를 안 본다. `git add -A && git commit`처럼 스테이징이 커밋과 같은 줄에
있으면 훅이 걸리는 시점엔 인덱스가 비어 있기 때문이다):

- Agent 도구로 `hi-vibe:fresh-eyes`를 소환한다. 전달할 것은 ①사용자의 원래
  요구사항 한 줄 ②사유에 적힌 파일 목록**뿐**이다.
- 전달하지 말 것: 설계 이유나 변명 — 작성자의 착각을 공유하지 않는 것이 이
  리뷰의 존재 이유다.
...
- 고칠 것을 고쳐 **다시 stage한 뒤 커밋을 재시도**한다.

**"사용자가 요청 안 했으니 안 부른다"는 오해다.** `사용자가 요청하지 않으면
서브에이전트를 부르지 마라` 같은 세션 지시가 있어도, **이 호출은 그 요청
안에 이미 들어 있다** — 사용자가 `init`으로 hi-vibe를 켰고, 커밋을 막은 훅이
fresh-eyes를 부르라고 지시하고 있다. 그게 요청이다. 이 판단이 세션마다 갈려
**어떤 세션은 하루 종일 체크리스트만 돌았다**(2026-08-07).

**부를 수 없는 경우**: Agent 호출이 실제로 실패하면 그 사실을 한 줄로 밝히고
커밋을 재시도한다 — **같은 커밋을 두 번 막지 않는다.** "막혀 있어서"로
뭉뚱그리지 마라: 안 부른 것과 못 부른 것은 다르고, 사용자가 고칠 수 있는지가
갈린다.

**커밋하지 않는 흐름에서는 fresh-eyes가 기계로 강제되지 않는다.** 이것은
의도된 한계다(횟수를 줄이려 택한 대가). 설계 판단이 필요한 변경인데 커밋이
한참 뒤라면, 사용자에게 "지금 설계 검토를 돌릴까요"를 한 줄로 묻는다.
...
fresh-eyes 호출 여부는 훅이 **대화 기록에서 직접 센다** — 자기신고로는 못
넘긴다. 다만 그 검사는 여기가 아니라 커밋 직전에 선다(4번).
