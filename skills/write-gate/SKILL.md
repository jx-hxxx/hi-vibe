...
2. 변경이 크면 줄 수 균형으로 나눠 병렬 리뷰하고, 작으면 순차로 본다.
3. 체크리스트에서 결함을 찾으면 요구사항 안의 결함만 고쳐 재검사한다.
4. 작지 않은 변경은 `fresh-eyes`를 실행해 파일 사이 어긋남과 판단 착오를 본다.
5. 실행 검증과 문서 동기화를 마친 뒤 `review_scope.py mark`로 완료를 표시한다.
6. 결과는 결함과 검증 근거 중심으로 짧게 보고한다.

사용자가 “가볍게 봐줘” 또는 “keep it light”라고 하면 병렬과 fresh-eyes를
생략하고 체크리스트만 적용한다. 생략 사실은 한 줄로 밝힌다.
