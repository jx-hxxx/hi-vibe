...
사용자가 "가볍게 봐줘"라고 했으면 병렬도 fresh-eyes도 생략하고 체크리스트만
돌린다 — 끄는 건 플래그가 아니라 말로 한다.
...
### 4. 남의 눈 (기본 켬 — 작을 때만 생략)

체크리스트를 마치면 **fresh-eyes 에이전트를 Agent 도구로 소환한다.** 이건
기본값이지 옵션이 아니다 — 체크리스트가 "빠뜨림"을 잡는다면 fresh-eyes는
"판단 착오"(과잉 설계, 스코프 크립, 더 단순한 길)를 잡고, 그건 작성자가
혼자서는 못 보는 것이다.
...
**생략하는 경우**: 변경이 작을 때만이다(오타·주석·포맷·한 줄 설정처럼
판단할 설계가 없는 것). 생략했으면 **조용히 넘기지 말고 한 줄로 밝힌다** —
"변경이 작아 설계 리뷰는 건너뜀". 무엇이 안 돌았는지 사용자가 알아야 한다.
