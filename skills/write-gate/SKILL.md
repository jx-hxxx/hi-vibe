...
1. **범위 계산**:
   ...
   → `to_review`(바뀐 코드 파일 중 아직/다시 봐야 할 것)와 `skipped`(이미
   리뷰했고 그 뒤로 안 바뀐 것)를 JSON으로 준다.
2. **비어 있으면**: `to_review`가 없으면 "새로 리뷰할 변경이 없습니다
   (이미 본 것 N개는 그대로)"만 알리고 끝낸다 — 억지로 훑지 않는다.
3. **점검**: `to_review`의 각 파일(=기능 덩어리)에 위 `Mode: review`
   체크리스트를 적용한다. **비례 원칙**: 큰 변경은 자세히, 작은 변경은
   핵심 항목만. 파일이 많으면 파일별 한두 줄 요약(✅/⚠️)으로 압축하고
   ⚠️만 펼친다. 실행 검증·문서 동기화(9·10)는 전체에 대해 마지막에 한 번
   종합해 답한다.
4. **건너뛴 것 표시**: `skipped`가 있으면 "이미 리뷰됨(변경 없음): N개"
   한 줄로 밝힌다.
5. **완료 기록 (필수)**: 실제로 리뷰를 마친 파일들을
