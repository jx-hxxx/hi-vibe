...
- **범용 병렬 서브에이전트 오케스트레이션은 넣지 않는다.** "서브에이전트가 유행"이라 올인원으로 얹자는 얘기가 나오지만: ①hi-vibe는 안전벨트(제약)지 생산성 액셀러레이터가 아니다 — "설계 빨리"는 Claude Code 기본 Task 도구로 이미 되며, 우리가 또 싸면 잡화점이 된다. ②병렬은 벽시계 시간만 줄이고 **총 토큰은 오히려 늘어난다**(시스템프롬프트·파일재독 N배). "토큰 절약"은 메인 컨텍스트 오염 방지일 뿐 총비용 절감이 아니며, 짧은 작업엔 순오버헤드다. ③이건 fresh-eyes가 잡을 전형적 과잉설계다. **유일한 fit은 `review --all --deep`에 fresh-eyes 팬아웃 하나뿐이고, 그마저도 소규모 타깃엔 근거가 약해 "review가 실제로 느리다"고 실측될 때까지 보류.**
